package com.Hileb.itzmx.item.skills;
...
import com.Hileb.itzmx.util.MessageDef;
...
import static com.Hileb.itzmx.util.CommonDef.G_SKY;
import static com.Hileb.itzmx.util.CommonDef.TICK_PER_SECOND;
import static com.Hileb.itzmx.util.CommonFunctions.SendMsgToPlayer;
import static com.Hileb.itzmx.util.CommonFunctions.isShiftPressed;
import static com.Hileb.itzmx.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "itzmx.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("itzmx.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("itzmx.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("itzmx.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("itzmx.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("itzmx.skill.shared.lv_max") : "";

        return I18n.format("itzmx.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("itzmx.skill.shared.duration_desc", val);
    }
