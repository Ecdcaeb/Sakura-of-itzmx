...
import static com.Hileb.itzmx.util.NBTStrDef.IDLNBTDef.BIOMETAL_WARNED;
		...
		if (!worldIn.isRemote)
        {
            if (((float)stack.getItemDamage() / stack.getMaxDamage()) >= 0.9f)
            {
                if (worldIn.getWorldTime() % TICK_PER_SECOND == 0 && !IDLNBTUtil.GetBoolean(stack, BIOMETAL_WARNED, false)) {
                    ...
                    CommonFunctions.SendMsgToPlayer((EntityPlayerMP) player, "itzmx.msg.low_dura", stack.getDisplayName());
                }
            }
            else {
...
//
//        //tooltip.add(I18n.format("itzmx.gua_enhance_total.desc", IDLSkillNBT.GetGuaEnhanceTotal(stack)));
